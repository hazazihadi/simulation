...
class TrafficSim(gym.Env):
    def __init__(self):
        ...
        #action
        self.n_action = 2**2 #different combinations of signal phases for the 2 intersections
        self.action_space = gym.spaces.Discrete(self.n_action)

        #state
        self.n_state = 2*4 # incoming links for all junctions
    ...
    def reset(self):
        ...
        #network definition
        J1 = W.addNode("J1", 1, 1, signal=[60,60,60,60])
        J2 = W.addNode("J2", 2, 1, signal=[60,60,60,60])
        WW = W.addNode("W1", 0, 1)
        EE = W.addNode("E1", 3, 1)
        N1 = W.addNode("N1", 1, 2)
        N2 = W.addNode("N2", 2, 2)
        ...
        W.addLink("N1_J1", N1, J1, length=500, free_flow_speed=10, jam_density=0.2, signal_group=1)
        W.addLink("SS1_J1", S1, J1, length=500, free_flow_speed=10, jam_density=0.2, signal_group=1)
        W.addLink("J2_J1", J2, J1, length=500, free_flow_speed=10, jam_density=0.2, signal_group=0)
        W.addLink("J1_W", J1, WW, length=500, free_flow_speed=10, jam_density=0.2)
        W.addLink("J1_N1", J1, N1, length=500, free_flow_speed=10, jam_density=0.2)
        W.addLink("J1_SS1", J1, S1, length=500, free_flow_speed=10, jam_density=0.2)
        W.addLink("J1_J2", J1, J2, length=500, free_flow_speed=10, jam_density=0.2, signal_group=0)
        W.addLink("N2_J2", N2, J2, length=500, free_flow_speed=10, jam_density=0.2, signal_group=1)
        W.addLink("EE_J2", EE, J2, length=500, free_flow_speed=10, jam_density=0.2, signal_group=0)
        W.addLink("S2_J2", S2, J2, length=500, free_flow_speed=10, jam_density=0.2, signal_group=1)
        W.addLink("J2_N2", J2,N2, length=500, free_flow_speed=10, jam_density=0.2)
        W.addLink("J2_EE", J2, EE, length=500, free_flow_speed=10, jam_density=0.2)
        W.addLink("J2_S2", J2, S2, length=500, free_flow_speed=10, jam_density=0.2)

        # Demand definition: Flow between J1 and J2 and external nodes
        dt = 30
        for t in range(0, 3600, dt):
            W.adddemand(S1, N1, t, t + dt,  volume=20)    # East of J2 to West of J1
            W.adddemand(N1, S1, t, t + dt,  volume=20)    # East of J2 to West of J1

        # store UXsim object for later re-use
        self.W = W
        self.I1 = J1
        self.I2 = J2
        self.INLINKS = list(self.I1.inlinks.values()) + list(self.I2.inlinks.values())
        print(self.INLINKS)
...
###############################################################################################
    ...
    def step(self, action_index):
        ...
        i1, i2 = int(binstr[3]), int(binstr[2])
        print(f"junc1: {i1}", f" junc2: {i2}")
        print(binstr)
        # i1, i2, i3, i4 = int(binstr[3]), int(binstr[2]),int(binstr[1]), int(binstr[0]) 
        # print(f"junc1: {i1}", f" junc2: {i2}", f"i3: {i3}", f"i4: {i4}")
...
for i_episode in range(num_episodes):
    ...
    if i_episode%50 == 0 or i_episode == num_episodes-1:
        ...
        env.W.analyzer.network_fancy(animation_speed_inverse=3, sample_ratio=1, interval=5, trace_length=3, figsize=6, antialiasing=False)
